...
XBot::JointNameMap NSPG::generateRandomVelocities(std::vector<XBot::ModelChain> colliding_chains){
    ...
    if(SCENARIO != 3) random_map.insert(std::make_pair("VIRTUALJOINT_1", generateRandom()*100));
    else random_map.insert(std::make_pair("VIRTUALJOINT_1", -fabs(generateRandom()*100)));
...
//    return false;
    ...
    if (check)
    {
        ...
        {
            if (sqrt(force.second(0)*force.second(0) + force.second(1)*force.second(1) + force.second(2)*force.second(2)) < 40.0)
            {
                check = false;
                break;
            }
